...
class AMDCore(ICTSetup):
    ...
    def evaluate(self, ctx: ICTContext) -> Optional[TradeSignal]:
        ...
        # (cheap gate FIRST, before building the heavier cycle window).
        ...
        # --- AMD cycle window -----------------------------------------------
        # The accumulation session (e.g. Asian 20:00-00:00 ET) closes HOURS
        # before the distribution entry (London / NY). A fixed tail() window
        # never reaches back to it, so slice from the CURRENT cycle's
        # accumulation start (most recent accumulation_session start at/<= ts)
        # forward: session_range then sees exactly ONE accumulation session and
        # the sweep/MSS/FVG span the full accumulation->manipulation->
        # distribution leg.
        df = self._cycle_window(exec_df, ts)
    ...
    # Steps 2-4 helpers
    # ------------------------------------------------------------------
    def _cycle_window(self, exec_df: "pd.DataFrame", ts) -> "pd.DataFrame":
        """Slice ``exec_df`` from the most recent accumulation-session START
        (e.g. 20:00 ET) at or before ``ts`` to the end - bounding the AMD
        sequence to a SINGLE cycle so ``session_range`` sees one accumulation
        session (not several nights stacked) and the sweep/MSS/FVG span the
        full accumulation->manipulation->distribution leg. Falls back to
        ``tail(_EXEC_SCAN_BARS)`` when the start can't be resolved or the slice
        is too small."""
        try:
            hh, mm = (int(x) for x in str(self.accumulation_session[0]).split(":"))
            t = pd.Timestamp(ts)
            if t.tz is None:
                t = t.tz_localize("UTC")
            et = t.tz_convert("US/Eastern")
            start_et = et.normalize() + pd.Timedelta(hours=hh, minutes=mm)
            if start_et > et:
                start_et -= pd.Timedelta(days=1)
            win = exec_df[exec_df.index >= start_et.tz_convert("UTC")]
            if len(win) >= _SWING_LOOKBACK * 2 + 2:
                return win
        except Exception as exc:
            logger.warning(f"[ict:amd:{self.variant}] cycle-window failed ({exc!r}); using tail")
        return exec_df.tail(_EXEC_SCAN_BARS)
